...
import Project, { getAllDbProjects, getAllProjects, RefreshProjectCache } from "../../models/Project";
    ...
    return new Promise<void>((resolve, reject) => {
        getAllDbProjects()
            .then(async (allProjects) => {
                const projects = allProjects.filter(x => x.appName == projectRequestData.appName);
                ...
                const userCanModify = projects[0].userProjects.filter(userProject => userProject.isOwner && userProject.user.discordId == discordId).length > 0 || isMod;
